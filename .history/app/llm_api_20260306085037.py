...
def refute_argument(config, topic_title, side, enhanced_argument):
    ...
    """
    # 随机选择一个反驳模型
    refute_models = config["REFUTE_MODELS"]
    chosen_model = random.choice(refute_models)
    ...
    result = call_llm_api(
        api_url=chosen_model["api_url"],
        api_key=chosen_model["api_key"],
        model=chosen_model["model"],
        messages=messages,
    )

    return result, chosen_model["name"]
