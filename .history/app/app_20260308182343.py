...
def submit_debate():
    ...
    # 辩题对应的测评命中立场（questions 中的 hit 字段，如 "side_a" / "side_b"）
    topic_hit = topic.get('hit')
...
@app.route('/storybook/<session_id>')
...
def generate_storybook(session_id):
    ...
    print(f"[绘本生成] force_new={force_new}")

    session = DebateSession.query.get_or_404(session_id)
    print(f"[绘本生成] 找到会话: {session.id}")
    ...
    try:
        ...
        try:
            ...
            db.session.rollback()
        print(f"[绘本生成错误] {e}")
        return jsonify({"success": False, "message": f"绘本生成失败: {str(e)[:200]}"}), 500
...
def get_storybook(storybook_id):
    """获取绘本详情"""
